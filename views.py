...
def new_questionnaire_part1():
    ...
    if request.method == 'POST':
        for i in range(1, 6):
            value = request.form.get(f'attitude{i}')
            # If user didn't move slider, use default value of 0
            session[f'attitude{i}'] = value if value is not None else '0'
...
def new_questionnaire():
    ...
    if request.method == 'POST':
        try:
            # 1) collect part1 from session and CONVERT to int
            combined = {}
            for i in range(1, 6):
                key = f'attitude{i}'
                ...
                if key in session:
                    try:
                        combined[key] = int(session[key])
                    except (ValueError, TypeError):
                        combined[key] = 0  # Default to 0 if conversion fails
            
            # 2) add form values (match1..match10 and any other fields)
            form_dict = request.form.to_dict()
            # Convert match values to int, default to 0 if not provided
            for i in range(1, 11):
                key = f'match{i}'
                if key in form_dict:
                    try:
                    ...
                    form_dict[key] = 0  # Default if not in form
            
            combined.update(form_dict)
            
            # 3) Save attitude1..attitude5 to User model
            for i in range(1, 6):
                key = f'attitude{i}'
                if key in combined:
                    setattr(current_user, key, int(combined[key]))
            
            # 4) Save match1..match10 to User model
            for i in range(1, 11):
                key = f'match{i}'
                if key in combined:
                    setattr(current_user, key, int(combined[key]))
            
            # 5) COMMIT the individual values FIRST
            db.session.commit()
            
            # 6) Use helper to calculate openness and check eligibility
