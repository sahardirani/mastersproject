...
def save_questionnaire_responses(user_id, form_data):
    """
    Save responses and calculate openness score
    No longer saves to UserOpinion table (matching uses users columns directly)
    """
    from .models import User
    ...
    # Process attitude questions (1-5) - saved to User model
    for i in range(1, 6):
        field_name = f'attitude{i}'
        if field_name in form_data:
            score = float(form_data[field_name])
            setattr(user, field_name, int(score))
            attitude_scores.append(score)
    
    # Process matching questions (1-10) - saved to User model
    for i in range(1, 11):
    ...
    if attitude_scores:
        ...
        user.is_extremist = openness_score < 0.0
